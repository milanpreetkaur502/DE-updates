...
debug_log () {
    	echo "debug_log"
	CGATT=NULL
        CGATT=$(echo -ne "AT+CGATT?\r\n" | microcom -t 1000 -X /dev/ttyUSB2 -s 115200)
        echo $CGATT
	COPS=NULL
        COPS=$(echo -ne "AT+COPS?\r\n" | microcom -t 1000 -X /dev/ttyUSB2 -s 115200)
        echo $COPS
	CGDCONT=NULL
        CGDCONT=$(echo -ne "AT+CGDCONT?\r\n" | microcom -t 1000 -X /dev/ttyUSB2 -s 115200)
        echo $CGDCONT
        
        # writing to the file
	echo "$CGATT $COPS $CGDCONT" > $DEBUG_MODEM
}
setup_gps () {  
#     printf "setup func\n"
